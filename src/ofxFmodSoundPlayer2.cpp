...
static vector<string> ofxFmodDeviceNames;
...
void ofxFmodSoundPlayer2::initializeFmod() {
	if (!bFmodInitialized_) {
		...
		if (listCreated == false) {
			...
			ofLogVerbose() << "--------------------------------";
			ofLogVerbose() << "FMOD: Available Sound Outputs:  ";
			for (int i = 0; i < ofxFmodNumDevices; i++) {
				ofLogVerbose() << i << " " << ofxFmodDeviceNames[i];
			}
			ofLogVerbose() << "--------------------------------";
		}
		...
		if (ofxFmodPreferedDevice < ofxFmodNumDevices) {
			...
			ofLogVerbose() << "Selected device = " << ofxFmodDevice << endl;
		}
		else {
			ofLogWarning() << "Warning: selected device not available, use default" << endl;
			ofxFmodDevice = 0;
		}
